...
var _ BatchablePriceProvider = &WasabiLPPriceProvider{}
...
func (w *WasabiLPPriceProvider) Initialize(ctx context.Context, client bind.ContractBackend, httpClient fetchers.HttpClient) error {
	var err error
...
func (w *WasabiLPPriceProvider) LPTokenPrice(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("wasabi: totalAssets: %w", err)
	}
...
func (w *WasabiLPPriceProvider) PriceReads() ([]multicall3.Call3, error) {
	abi, err := sc.ERC4626MetaData.GetAbi()
	if err != nil {
		return nil, fmt.Errorf("wasabi: ABI: %w", err)
	}
	tsData, err := abi.Pack("totalSupply")
	if err != nil {
		return nil, fmt.Errorf("wasabi: pack totalSupply: %w", err)
	}
...
func (w *WasabiLPPriceProvider) ComputePrice(responses []multicall3.Result3) (decimal.Decimal, error) {
	if len(responses) != 2 {
		return decimal.Zero, fmt.Errorf("wasabi: expected 2 responses, got %d", len(responses))
	}
	for i, r := range responses {
		if !r.Success {
			return decimal.Zero, fmt.Errorf("wasabi: sub-call %d reverted", i)
		}
	}
	abi, err := sc.ERC4626MetaData.GetAbi()
	if err != nil {
		return decimal.Zero, fmt.Errorf("wasabi: ABI: %w", err)
	}
	tsOut, err := abi.Methods["totalSupply"].Outputs.Unpack(responses[0].ReturnData)
	if err != nil {
		return decimal.Zero, fmt.Errorf("wasabi: unpack totalSupply: %w", err)
	}
	totalSupply, ok := tsOut[0].(*big.Int)
	if !ok {
		return decimal.Zero, fmt.Errorf("wasabi: totalSupply type %T", tsOut[0])
	}
	taOut, err := abi.Methods["totalAssets"].Outputs.Unpack(responses[1].ReturnData)
	if err != nil {
		return decimal.Zero, fmt.Errorf("wasabi: unpack totalAssets: %w", err)
	}
	totalAssets, ok := taOut[0].(*big.Int)
	if !ok {
		return decimal.Zero, fmt.Errorf("wasabi: totalAssets type %T", taOut[0])
	}
	return w.computeLPPriceFromReads(totalSupply, totalAssets)
}

func (w *WasabiLPPriceProvider) computeLPPriceFromReads(totalSupply, totalAssets *big.Int) (decimal.Decimal, error) {
	if totalSupply.Sign() == 0 {
		err := fmt.Errorf("total supply is zero")
		w.logger.Error().Err(err).Msg("invalid totalSupply")
		return decimal.Zero, err
	}
	assetPrice, err := w.getPrice(w.config.Token0)
	if err != nil {
		return decimal.Zero, err
	}
