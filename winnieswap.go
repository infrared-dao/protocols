...
var _ BatchablePriceProvider = &WinnieSwapLPPriceProvider{}
...
func (w *WinnieSwapLPPriceProvider) PriceReads() ([]multicall3.Call3, error) {
	abi, err := sc.StickyVaultMetaData.GetAbi()
	if err != nil {
		return nil, fmt.Errorf("winnieswap: ABI: %w", err)
	}
	tsData, err := abi.Pack("totalSupply")
	if err != nil {
		return nil, fmt.Errorf("winnieswap: pack totalSupply: %w", err)
	}
	gubData, err := abi.Pack("getUnderlyingBalances")
	if err != nil {
		return nil, fmt.Errorf("winnieswap: pack getUnderlyingBalances: %w", err)
	}
	return []multicall3.Call3{
		{Target: w.address, AllowFailure: false, CallData: tsData},
		{Target: w.address, AllowFailure: false, CallData: gubData},
	}, nil
}

func (w *WinnieSwapLPPriceProvider) ComputePrice(responses []multicall3.Result3) (decimal.Decimal, error) {
	if len(responses) != 2 {
		return decimal.Zero, fmt.Errorf("winnieswap: expected 2 responses, got %d", len(responses))
	}
	for i, r := range responses {
		if !r.Success {
			return decimal.Zero, fmt.Errorf("winnieswap: sub-call %d reverted", i)
		}
	}
	abi, err := sc.StickyVaultMetaData.GetAbi()
	if err != nil {
		return decimal.Zero, fmt.Errorf("winnieswap: ABI: %w", err)
	}
	tsOut, err := abi.Methods["totalSupply"].Outputs.Unpack(responses[0].ReturnData)
	if err != nil {
		return decimal.Zero, fmt.Errorf("winnieswap: unpack totalSupply: %w", err)
	}
	totalSupply, ok := tsOut[0].(*big.Int)
	if !ok {
		return decimal.Zero, fmt.Errorf("winnieswap: totalSupply type %T", tsOut[0])
	}
	gubOut, err := abi.Methods["getUnderlyingBalances"].Outputs.Unpack(responses[1].ReturnData)
	if err != nil {
		return decimal.Zero, fmt.Errorf("winnieswap: unpack getUnderlyingBalances: %w", err)
	}
	if len(gubOut) != 2 {
		return decimal.Zero, fmt.Errorf("winnieswap: getUnderlyingBalances returned %d fields, want 2", len(gubOut))
	}
	a0, ok := gubOut[0].(*big.Int)
	if !ok {
		return decimal.Zero, fmt.Errorf("winnieswap: amount0 type %T", gubOut[0])
	}
	a1, ok := gubOut[1].(*big.Int)
	if !ok {
		return decimal.Zero, fmt.Errorf("winnieswap: amount1 type %T", gubOut[1])
	}
	return w.computeLPPriceFromReads(totalSupply, a0, a1)
}

func (w *WinnieSwapLPPriceProvider) computeLPPriceFromReads(totalSupply, amount0, amount1 *big.Int) (decimal.Decimal, error) {
...
func (w *WinnieSwapLPPriceProvider) GetConfig(ctx context.Context, address string, client bind.ContractBackend) ([]byte, error) {
	var err error
