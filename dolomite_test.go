...
func TestDolomite_BatchPathMatchesLegacy(t *testing.T) {
	t.Parallel()
	assetPrice, _ := decimal.NewFromString("1.10")
	d := &DolomiteLPPriceProvider{
		address: common.HexToAddress("0x4444444444444444444444444444444444444444"),
		logger:  zerolog.Nop(),
		config:  &DolomiteConfig{Token0: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", LPTDecimals: 18},
	...
	totalAssets := new(big.Int).Mul(big.NewInt(220), bigPow10(18))
	wantDirect, _ := d.computeLPPriceFromReads(totalSupply, totalAssets)
	...
	got, err := d.ComputePrice([]multicall3.Result3{
